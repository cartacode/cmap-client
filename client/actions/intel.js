...
import { INTEL_EEI__ADD, INTEL_REQUEST__ADD, INTEL_EEI__FETCH, INTEL_REQUEST__FETCH, INTEL_REQUEST__FETCH_ONE, INTEL_REQUEST__UPDATE, INTEL_REQUEST__DELETE, INTEL_APPROVED_VALIDATED__STATUS_UPDATE, INTEL_REQUEST_COLL_MANAGER__UPDATE, INTEL_REQUEST__COPY } from 'dictionary/action';
...
export function updateIntelRequestWithCollectionManager(intelReqId,intelRequest) {
  return createAction({
    type: INTEL_REQUEST_COLL_MANAGER__UPDATE,
    action: () => axios.put(`${baseUrl}/IntelRequest/UpdateIntelRequestWithCollectionManager?intelReqId=${intelReqId}`, JSON.stringify(intelRequest), {headers:requestHeaders}),
  });
}
